...
# Replace concept_explainer logic
new_logic = """
    import asyncio

    async def generate_candidate(i: int):
        varied_query = f"{search_query} (Variant {i+1}: Explain this concept from a slightly different perspective.)"
    ...
    tasks = [generate_candidate(i) for i in range(3)]  # Default 3 candidates for teacher explainer
    ...
    return BestScoredSummaryResponse(
        ...
        scores=PipelineScores(
            entailment=bool(winner.grounding >= 0.5),
            consistency=bool(winner.consistency >= 0.5),
...
with open('backend/app/api/v1/teacher.py', 'w') as f:
    f.write(content)
print("Updated teacher.py")
